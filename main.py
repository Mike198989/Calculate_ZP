...
def main(page: ft.Page):
    ...
    def auto_recalculate(e=None):
        ...
        for label, val, is_bold in details_items:
            color = "red300" if val < 0 else ("blue200" if is_bold else "white70")
    ...
    shifts_tab_content = ft.Column(
        controls=[
            ft.Text("Параметры смен", size=16, weight="bold", color="blue300"),
    ...
    oklad_tab_content = ft.Column(
        controls=[
            ft.Text("Параметры оклада", size=16, weight="bold", color="blue300"),
    ...
    settings_tab_content = ft.Column(
        controls=[
            ft.Text("Ставки и коэффициенты", size=16, weight="bold", color="blue300"),
    ...
    def update_history_view():
        ...
        if not history:
            history_list_view.controls.append(
                ft.Text("История расчётов пуста", color="grey500", italic=True)
            )
            page.update()
            return
                    ...
                    content=ft.Container(
                        content=ft.Row(
                            controls=[
                                ft.Column(
                                    controls=[
                                        ft.Text(f"{mode_title} — {net_val}", weight="bold", size=15),
    ...
    results_card = ft.Card(
        ...
        content=ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("На руки (Net):", size=14, color="grey300"),
                    net_output,
                    payout_output,
    ...
    page.add(
        ft.Column(
            controls=[
                ft.Row(
                    controls=[
                        ft.Text("🧮 Калькулятор ЗП", size=20, weight="bold"),
                    ],
                    alignment="center",
                ),
                nav_tabs,
